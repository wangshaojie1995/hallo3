...
def cross_attention_forward_default(self, hidden_states, cross_attention_mask, encoder_outputs, **kw_args):
    ...
    is_face = kw_args.get('is_face', False)
    
    assert (is_audio and is_face) == False
    
    if is_audio:
        self = self.transformer.layers[kw_args['layer_id']].audio_attn
    elif is_face:
        self = self.transformer.layers[kw_args['layer_id']].face_attn
    else:
        raise ValueError("one of is_audio and is_face should be true")
