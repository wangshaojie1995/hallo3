...
class VideoDDIMSampler(BaseDiffusionSampler):
    ...
    def denoise(self, x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None,
                audio_emb=None, face_emb=None):
        additional_model_inputs = {"audio_emb": audio_emb,
                                   "face_emb": face_emb}
        ...
        if isinstance(scale, torch.Tensor) == False and scale == 1:
            ...
            uc_audio_emb = torch.zeros_like(audio_emb)
            additional_model_inputs["audio_emb"] = torch.cat([uc_audio_emb, audio_emb], dim=0)
            
            # uc_face_emb = torch.zeros_like(face_emb)
            additional_model_inputs["face_emb"] = torch.cat([face_emb, face_emb], dim=0)
            ...
            if isinstance(self.guider, DynamicCFG):
                ...
                # uc, c_text, c_audio = denoised.chunk(3)
                # text_scale = 3.5
                # audio_scale = 3.5
                # denoised = uc + text_scale * (c_text - uc) + audio_scale * (c_audio - uc)
                # uc, c = denoised.chunk(2)
                # scale = 3.5
                # denoised = uc + scale * (c - uc) 
