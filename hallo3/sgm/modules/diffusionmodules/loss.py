...
class VideoDiffusionLoss(StandardDiffusionLoss):
    ...
    def __call__(self, network, ref_network, denoiser, conditioner, input, batch):
        ...
        # ref_cond['concat'] = batch['ref_image']
        ref_cond['concat'] = batch['mask_ref']
        ref_timestep = torch.Tensor([0]).to(input.device)
        # ref_network(batch['ref_image'], ref_timestep, ref_cond)
        ref_network(batch['mask_ref'], ref_timestep, ref_cond)
        num_layer = len(ref_network.diffusion_model.transformer.layers)
        for i in range(num_layer):
            network.diffusion_model.transformer.layers[i].bank = [
                v.clone() for v in ref_network.diffusion_model.transformer.layers[i].bank
            ]
            ref_network.diffusion_model.transformer.layers[i].bank.clear()
