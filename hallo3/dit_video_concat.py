...
class ImagePatchEmbeddingMixin(BaseMixin):
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        emb = emb.contiguous()
        return emb  # (b,n_t+t*n_i+n_ref+n_face,d)
...
class AdaLNMixin(BaseMixin):
    ...
    def layer_forward(
        ...
        **kwargs,
        ...
        if ref_feat is None:
            assert len(layer.bank) == 1
            ref_feat = layer.bank[0]
        ref_length = ref_feat.shape[1]
        ...
        attention_input = torch.cat((text_attention_input, img_attention_input, ref_feat), dim=1)  # (b,n_t+t*n_i,d)
        attention_output = layer.attention(attention_input, mask, **kwargs)
        
        text_attention_output = attention_output[:, :text_length]  # (b,n,d)
        img_attention_output = attention_output[:, text_length:-ref_length]  # (b,(t n),d)
        ...
        # face attn
        face_attn_input = layer.face_input_layernorm(img_hidden_states)  # vision (b,(t n),d)
        face_attn_input = rearrange(face_attn_input, "b (t n) d -> (b t) n d", t=video_length)
        kwargs['encoder_outputs'] = face_emb
        kwargs['is_cross'] = True
        kwargs['is_face'] = True
        face_attn_output = layer.face_attn(face_attn_input, mask, **kwargs)
        face_attn_output = rearrange(face_attn_output, "(b t) n d -> b (t n) d", t=video_length)
        img_hidden_states = img_hidden_states + face_attn_output
        kwargs['is_cross'] = False
        kwargs['is_face'] = False
        ...
        if layer.add_audio_module: 
            ...
            img_hidden_states = img_hidden_states + audio_attn_output
            kwargs['is_cross'] = False
...
class DiffusionTransformer(BaseModel):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if self.add_audio_module:
            kwargs["audio_emb"] = kwargs["audio_emb"].to(self.dtype)
        else:
            kwargs["audio_emb"] = None
            
        kwargs["face_emb"] = kwargs["face_emb"].to(self.dtype)
